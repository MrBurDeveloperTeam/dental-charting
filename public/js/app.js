...
function removeSelectedEntries(ids){const idSet=new Set(ids);Object.values(activeState()).forEach(toothState=>{toothState.entries=toothState.entries.filter(entry=>!idSet.has(entry.id))});if(editingEntry&&idSet.has(editingEntry.id))editingEntry=null;selectedEntryIds.clear();renderAll();document.dispatchEvent(new CustomEvent("dental-chart:delete-entries",{detail:{ids}}))}
...
document.getElementById("entries-delete-selected")?.addEventListener("click",()=>{const ids=[...selectedEntryIds];if(!ids.length)return;openDeleteEntryModal(`Delete ${ids.length} selected entr${ids.length===1?"y":"ies"}? This cannot be undone.`,()=>removeSelectedEntries(ids))});
...
function saveDraft(){
  ...
  const savedEntries=[];
  targets.forEach(n=>{
    const entry={id:editingEntry&&editingEntry.tooth===n?editingEntry.id:uid(),tooth:n,treatment:draft.treatment,category:draft.category,status:draft.status,layer:draft.status==="watch"?draft.layer:(draft.status==="planned"?"planned":"existing"),view:draft.view,surfaces:[...draft.surfaces],note:draft.note.trim()};
    const entries=activeState()[n].entries,index=editingEntry&&editingEntry.tooth===n?entries.findIndex(item=>item.id===editingEntry.id):-1;
