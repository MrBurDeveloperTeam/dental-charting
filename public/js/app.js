...
const permanentLayout={upperDisplay:[...DENTITIONS.permanent.upperDisplay],lowerDisplay:[...DENTITIONS.permanent.lowerDisplay]};
...
const COLORS={composite:"#15803d",amalgam:"#1d4ed8",gic:"#ef4444",sealant:"#55a8e8",caries:"#4b5563",rootCaries:"#ae7042",fracture:"#bd4a2e",missing:"#a8b4c1",extraction:"#d85852",implant:"#0f9fa8",rootCanal:"#d85852",bridge:"#6366f1",partialDenture:"#ec7890",crown:"#8c25f4",veneer:"#00fbff"};
const TREATMENTS={composite:{label:"Composite",category:"restoration",mode:"surface",views:["occ","front"]},amalgam:{label:"Amalgam",category:"restoration",mode:"surface",views:["occ","front"]},gic:{label:"GIC",category:"restoration",mode:"surface",views:["occ","front"]},sealant:{label:"Sealant",category:"restoration",mode:"surface",views:["occ"]},caries:{label:"Caries",category:"condition",mode:"surface",views:["occ","front"]},rootCaries:{label:"Root caries",category:"condition",mode:"surface",views:["front"]},fracture:{label:"Fracture",category:"condition",mode:"surface",views:["occ","front"]},missing:{label:"Missing",category:"condition",mode:"whole",views:["occ","front"]},rootCanal:{label:"Root canal",category:"procedure",mode:"root",views:["front"]},extraction:{label:"Extraction",category:"procedure",mode:"whole",views:["occ","front"]},implant:{label:"Implant",category:"procedure",mode:"whole",views:["occ","front"]},bridge:{label:"Bridge",category:"prosthetic",mode:"whole",views:["occ","front"]},partialDenture:{label:"Partial denture",category:"prosthetic",mode:"whole",views:["occ","front"]},crown:{label:"Crown",category:"prosthetic",mode:"whole",views:["occ","front"]},veneer:{label:"Veneer",category:"prosthetic",mode:"whole",views:["occ","front"]}};
const CATEGORIES=[{id:"condition",label:"Condition"},{id:"restoration",label:"Restoration"},{id:"procedure",label:"Procedure"},{id:"prosthetic",label:"Prosthetic"}];
for(const id of ['composite','amalgam','gic','rootCaries'])TREATMENTS[id].visible=false;
Object.assign(TREATMENTS, {
  filling:{label:'Filling',category:'restoration',mode:'surface',views:['occ','front'],requiresMaterial:true},
  inlay:{label:'Inlay',category:'restoration',mode:'surface',views:['occ','front'],requiresMaterial:true},
  onlay:{label:'Onlay',category:'restoration',mode:'surface',views:['occ','front'],requiresMaterial:true},
  overlay:{label:'Overlay',category:'restoration',mode:'surface',views:['occ','front'],requiresMaterial:true},
  ...
  impacted:{label:'Impacted',category:'condition',mode:'label',views:['occ','front'],badge:'IMP'},
  kiv:{label:'KIV',category:'condition',mode:'label',views:['occ','front'],badge:'KIV'}
});
Object.assign(COLORS,{caries:'#000000',crack:'#facc15',fracture:'#f97316',extraction:'#ef4444',filling:'#ffe0ad',inlay:'#ffe0ad',onlay:'#ffe0ad',overlay:'#ffe0ad',spacing:'#55a8e8',impacted:'#8b5cf6',m1:'#fb7185',m2:'#fb7185',m3:'#fb7185',kiv:'#fbbf24'});
const STATUSES=[{id:"existing",label:"Existing"},{id:"planned",label:"Planning"}];
...
// Legacy treatment customization is retained below, but no longer loaded.
const state={permanent:{},primary:{}};
[...DENTITIONS.permanent.upper,...DENTITIONS.permanent.lower,...DENTITIONS.primary.upper,...DENTITIONS.primary.lower].forEach(n=>state.permanent[n]={entries:[]});
...
const draft={tooth:null,category:"condition",treatment:"caries",material:null,view:"occ",status:"existing",layer:"existing",surfaces:[],note:""};
...
function closeDeleteEntryModal(){deleteEntryModal.classList.remove("show");deleteEntryModal.setAttribute("aria-hidden","true");pendingDeleteAction=null}
function openDeleteEntryModal(message,action){deleteEntryMessage.textContent=message;pendingDeleteAction=action;deleteEntryModal.classList.add("show");deleteEntryModal.setAttribute("aria-hidden","false");setTimeout(()=>document.getElementById("delete-entry-cancel")?.focus(),0)}
function removeSelectedEntries(ids){const groups=new Set(flatEntries().filter(e=>ids.includes(e.id)&&e.bridgeId).map(e=>e.bridgeId));ids=[...new Set([...ids,...flatEntries().filter(e=>groups.has(e.bridgeId)).map(e=>e.id)])];const idSet=new Set(ids);Object.values(activeState()).forEach(toothState=>{toothState.entries=toothState.entries.filter(entry=>!idSet.has(entry.id))});if(editingEntry&&idSet.has(editingEntry.id))editingEntry=null;selectedEntryIds.clear();renderAll();document.dispatchEvent(new CustomEvent("dental-chart:delete-entries",{detail:{ids}}))}
document.getElementById("delete-entry-close")?.addEventListener("click",closeDeleteEntryModal);
...
function closeFinishVisitModal(){finishVisitModal.classList.remove("show");finishVisitModal.setAttribute("aria-hidden","true")}
document.getElementById("finish-visit-close")?.addEventListener("click",closeFinishVisitModal);
...
els.dateView.addEventListener("click",e=>{e.stopPropagation(); const btn=e.target.closest("[data-date-action]"); if(!btn) return; const action=btn.dataset.dateAction; if(action==="select-day"){selectDateValue(btn.dataset.iso); return} if(action==="select-month"){datePicker.cursor=`${monthStartFromKey(datePicker.cursor).getFullYear()}-${`${Number(btn.dataset.month)+1}`.padStart(2,"0")}`; datePicker.view="day"; renderDatePopover(); positionDatePopover(); return} if(action==="select-year"){const base=monthStartFromKey(datePicker.cursor); datePicker.cursor=`${btn.dataset.year}-${`${base.getMonth()+1}`.padStart(2,"0")}`; datePicker.view="month"; renderDatePopover(); positionDatePopover();}});
els.datePopover.addEventListener("click",e=>e.stopPropagation());
...
function treatmentFor(id){return TREATMENTS[id]||{label:id,category:"condition",mode:"label",views:["occ","front"]}}
function toothW(n){
...
function crownDims(n){
  const anatomy=innerAnatomy(n);
  if(anatomy)return {width:anatomy.width,height:anatomy.height};
...
function defaultTreatmentIcon(id){if(id==="rootCanal")return"canal";if(id==="crown")return"crown";if(id==="bridge")return"bridge";if(id==="partialDenture")return"partialDenture";if(id==="spacing")return"spacing";if(id==="veneer")return"veneer";if(id==="implant")return"implant";if(id==="extraction")return"extraction";if(id==="missing")return"cross";if(id==="sealant")return"seal";if(id==="caries"||id==="rootCaries")return"circle";if(id==="fracture")return"bolt";if(id==="crack")return"crack";if(id==="inlay")return"inlay";if(id==="onlay")return"onlay";if(id==="overlay")return"overlay";if(id==="impacted")return"imp";if(id==="retainedRoot")return"root";if(["m1","m2","m3","kiv"].includes(id))return id;return"filling"}
...
function activeUpperDisplay(){return chartMode==="primary"?primaryLayout.upperDisplay:permanentLayout.upperDisplay}
function activeLowerDisplay(){return chartMode==="primary"?primaryLayout.lowerDisplay:permanentLayout.lowerDisplay}
...
function currentToothName(n){return TOOTH_NAMES[chartMode][n]||TOOTH_NAMES.permanent[n]||TOOTH_NAMES.primary[n]||`Tooth ${n}`}
...
function swapPrimarySlot(n){
  const layout=chartMode==="primary"?primaryLayout:permanentLayout;
  const next=swapPrimaryToothNumber(n);
  if(!next) return null;
  const keys=["upperDisplay","lowerDisplay"];
  for(const key of keys){
    const index=layout[key].indexOf(n);
    if(index===-1) continue;
    layout[key][index]=next;
    return next;
  }
  return null;
}
...
function syncDateField(targetName){
  ...
  target.textInput.removeAttribute("aria-invalid");
  target.textInput.setCustomValidity("");
...
function commitDateField(targetName,{emptyOk=true,fallbackToday=false}={}){
  ...
  if(parsed===null){showDateFieldError(targetName); return false}
  if(parsed===""){
...
function innerAnatomy(n){return typeof INNER_ANATOMY!=="undefined"?(INNER_ANATOMY[`${chartMode}:${n}`]||INNER_ANATOMY[`${isPrimaryTooth(n)?"primary":"permanent"}:${n}`]):null}
let lowerCrownFilterSequence=0;
function photoToothSVG(n,view){
  ...
  const sharpen=view==="occ"&&!isUpper(n);
  const filterId=sharpen?`lower-crown-detail-${++lowerCrownFilterSequence}`:null;
  const detailFilter=sharpen?`<defs><filter id="${filterId}" x="0" y="0" width="100%" height="100%" color-interpolation-filters="sRGB"><feConvolveMatrix order="3" kernelMatrix="0 -0.15 0 -0.15 1.6 -0.15 0 -0.15 0" divisor="1" bias="0" preserveAlpha="true" edgeMode="duplicate"/></filter></defs>`:"";
  const image=`<image href="${asset}" x="0" y="0" width="${dims.width}" height="${dims.height}" preserveAspectRatio="none"${sharpen?` filter="url(#${filterId})"`:""}/>`;
  const content=view==="front"&&isUpper(n)?`<g transform="translate(0 ${dims.height}) scale(1 -1)">${image}</g>`:image;
  return `<svg class="photo-tooth-svg" viewBox="0 0 ${dims.width} ${dims.height}" width="${dims.width}" height="${dims.height}" aria-hidden="true">${detailFilter}${content}</svg>`;
}
...
// Crown regions stop at the neck: the photographed roots are context only.
// Geometry is in the same coordinates as the generated image/silhouette.
function innerSurfaceDefs(n){
  const {width:w,height:h}=crownDims(n),upper=isUpper(n);
  const anterior=["incisor","canine"].includes(toothType(n));
  const mesialOnRight=[1,4,5,8].includes(Math.floor(n/10));
  const top=upper?.035:.43,bottom=upper?(toothType(n)==="premolar"?.43:anterior?.51:.48):.97;
  const rect=(key,x1,y1,x2,y2)=>({key,path:`M${w*x1},${h*y1}H${w*x2}V${h*y2}H${w*x1}Z`,cx:w*(x1+x2)/2,cy:h*(y1+y2)/2});
  const keepRectangular=[13,12,11,21,22,23,42,41,32].includes(n);
  if(!keepRectangular){
    const crownTop=h*top,crownHeight=h*(bottom-top),y=ratio=>crownTop+crownHeight*ratio;
    const leftKey=mesialOnRight?"D":"M",rightKey=mesialOnRight?"M":"D";
    const result=[
      {key:leftKey,path:`M${w*.04},${y(.5)} C${w*.05},${y(.12)} ${w*.2},${y(.06)} ${w*.34},${y(.5)} C${w*.2},${y(.94)} ${w*.05},${y(.88)} ${w*.04},${y(.5)} Z`,cx:w*.18,cy:y(.5)},
  ...
  if(anterior){
    const edge=upper?.14:.86;
    result.push(rect("I",.24,upper?top:edge,.76,upper?edge:bottom));
...
function frontCrownSurfaceDefs(n){
  const w=toothW(n),end=toothH(n)*.43,side=w*.16;
  const swap=rightRootNeedsMDSwap(n,"front");
  const anterior=["incisor","canine"].includes(toothType(n));
  const region=(key,x,y,width,height)=>({key,path:`M${x},${y} H${x+width} V${y+height} H${x} Z`,cx:x+width/2,cy:y+height/2});
...
function surfaceLongLabel(code,n,v){
  ...
  if(code==="L") return isUpper(n)?"Palatal / lingual":"Lingual";
...
function treatmentHint(id){
  const hints={
    ...
    crown:"full crown",
    partialDenture:"removable denture",
    veneer:"facial shell"
  };
...
function treatmentIconMarkup(icon,color){
  ...
  if(icon==="cross")return svg(`<path d="M5 9 Q7 5 11 6 Q14 9 17 6 Q21 5 23 9 Q22 14 20 17 L18 25 Q16 27 15 21 Q14 17 13 21 Q12 27 10 25 L8 17 Q6 14 5 9Z" fill="none" stroke="#a8b0bc" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="3 3"/><path d="M7 3 L9 5 M14 1 V4 M21 3 L19 5" stroke="#a8b0bc" stroke-width="2.3" stroke-linecap="round"/><path d="M1 25.5 H6 M22 25.5 H27" stroke="#fb7f86" stroke-width="3" stroke-linecap="round"/>`);
  if(icon==="seal")return svg(`${tooth}<path d="M7.2 8 Q10 6.5 12.1 8.7 Q14 10.4 15.9 8.7 Q18 6.5 20.8 8 M14 9.7 V14" fill="none" stroke="#55a8e8" stroke-width="2.2" stroke-linecap="round"/>`);
  if(icon==="circle")return svg(`${tooth}<circle cx="16" cy="11" r="4.2" fill="${color}"/>`);
  if(icon==="bolt")return svg(`${tooth}<path d="M15 5 L10 13 H14 L11 23 L19 12 H15Z" fill="${color}"/>`);
  if(icon==="crack")return svg(`${tooth}<path d="M15 5 L11 11 L16 14 L12 23" fill="none" stroke="${color}" stroke-width="2.2" stroke-linecap="round"/>`);
  if(icon==="inlay")return svg(`${tooth}<path d="M10 5.3 Q14 8.2 18 5.3 L16.7 12.5 Q14 14.5 11.3 12.5Z" fill="#8d8886" stroke="#5f5a58" stroke-width="1.1"/>`);
  if(icon==="onlay")return svg(`${tooth}<path d="M5.8 7.5 Q8 4.5 11 5.2 Q14 8 17 5.2 L16.6 12.2 Q14 14.4 11.4 12.2 L10.5 9 Q8 8.2 6.2 10Z" fill="#8d8886" stroke="#5f5a58" stroke-width="1.1"/>`);
  if(icon==="overlay")return svg(`${tooth}<path d="M5.4 7.7 Q7.5 4.3 11 5 Q14 8 17 5 Q20.5 4.3 22.6 7.7 L21.3 11.8 Q17.5 14.3 14 13.8 Q10.5 14.3 6.7 11.8Z" fill="#8d8886" stroke="#5f5a58" stroke-width="1.1"/>`);
  if(icon==="impacted")return svg(`<path d="M5 19 L20 5 L24 9 L10 23 Z" fill="none" stroke="${color}" stroke-width="2"/><path d="M4 24 L24 4" stroke="${color}" stroke-width="1.5" stroke-dasharray="2 2"/>`);
  if(icon==="root")return svg(`<path d="M9 12 C7 11.5 5 10 5 8 Q5.5 4.5 9 4.5 Q11.5 4.5 14 6.8 Q16.5 4.5 19 4.5 Q22.5 4.5 23 8 C23 10 21 11.5 19 12" fill="rgba(168,168,168,.06)" stroke="#a8a8a8" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" pathLength="100" stroke-dasharray="10 7"/><path d="M9 12 Q11 14 14 12 Q17 14 19 12 L18 24 Q17.5 27 16 23 L15 18 Q14 15.5 13 18 L12 23 Q10.5 27 10 24Z" fill="#fffaf2" stroke="#20324d" stroke-width="1.8" stroke-linejoin="round"/><path d="M11.2 13.5 Q12 17 11.5 23 M16.8 13.5 Q16 17 16.5 23" fill="none" stroke="#ef5b58" stroke-width="2" stroke-linecap="round"/>`);
  if(icon==="watch")return svg(`<path d="M3 14 Q8 6 14 6 Q20 6 25 14 Q20 22 14 22 Q8 22 3 14Z" fill="none" stroke="${color}" stroke-width="2"/><circle cx="14" cy="14" r="4" fill="${color}"/>`);
  if(["m1","m2","m3","kiv","imp"].includes(icon))return svg(`<rect x="3" y="5" width="22" height="18" rx="4" fill="none" stroke="${color}" stroke-width="2"/><text x="14" y="18.2" text-anchor="middle" font-size="${icon==='kiv'||icon==='imp'?9:11}" font-weight="800" fill="${color}">${icon.toUpperCase()}</text>`);
  return svg(`${tooth}<path d="M11 6.4 Q14 8.5 17 6.4 L16.5 11.4 Q14 13 11.5 11.4Z" fill="#8d8886" stroke="#5f5a58" stroke-width="1"/><g stroke="#20324d" stroke-linecap="round" stroke-linejoin="round"><path d="M2.5 1.8 H8.5" stroke-width="1.6"/><path d="M4.5 1 V5 M2 5 H7" stroke-width="1.5"/><rect x="8" y="1.2" width="18" height="5" rx="2.2" fill="#fff" stroke-width="1.8"/><path d="M11 2.2 H23 Q24.5 2.2 24.5 3.7 Q24.5 5.2 23 5.2 H11Z" fill="#20b8c7" stroke="none"/><path d="M11.5 6.2 V8.2 Q11.5 9.2 12.5 9.2 H14 V10.8" fill="none" stroke-width="1.7"/></g><circle cx="14" cy="11.5" r="1" fill="#20b8c7"/>`);
}
...
function visibleEntrySurfaces(n,v,entry){
  ...
  if(entry.view!==v&&!treatment.views.includes(v))return [];
  const available=availableSurfaceCodes(n,v);
  return entry.surfaces.filter(surface=>available.includes(surface));
}
function selectedSurfaces(n,v){return new Set(draft.tooth===n?visibleEntrySurfaces(n,v,draft):[])}
function surfaceMap(n,v,s,layer=null){const map={}; entriesByStatus(n,s,layer).forEach(e=>visibleEntrySurfaces(n,v,e).forEach(sf=>map[sf]=e)); return map}
function isVeneer(t){return t==="veneer"}
function hideWholeRing(t){return t==="retainedRoot"||t==="bridge"||t==="partialDenture"||t==="crown"||t==="implant"||t==="rootCanal"||t==="extraction"}
function selectionSummary(){const viewText=draft.view==="occ"?"Crown + inner view":"Root view"; const surfaceText=draft.surfaces.length?` · ${draft.surfaces.join("/")} surface`:""; return `${viewText}${surfaceText}`}
...
function saveDraft(){
  ...
  const selectedTargets=selection.multi&&selection.teeth.length?selection.teeth:(draft.tooth?[draft.tooth]:[]);
  const targets=isGroupedProsthetic()?groupedTargets(selectedTargets):selectedTargets;
  if(!targets.length)return;
  normalizeDraft();
  if(materialSelectionError()){showChartValidation(materialSelectionError());return;}
  const bridgeError=isGroupedProsthetic()?groupedSelectionError(targets):"";
...
function completeMobileEntryWizard(){
  if(!draft.tooth||isGroupedProsthetic()&&groupedSelectionError(selection.teeth))return;
...
function renderMobileEntryWizard(){
  ...
  mobileEntryEls.done.disabled=Boolean(materialSelectionError())||isGroupedProsthetic()&&Boolean(groupedSelectionError(selection.teeth));
}
...
function renderSplitChartView(){
  els.splitStage.querySelectorAll("[data-chart-layer]").forEach(group=>group.classList.remove("view-hidden"));
...
function removeEntry(tooth,id){
  ...
  if(toothState.entries.find(e=>e.id===id)?.bridgeId){removeSelectedEntries([id]);return}
  toothState.entries=toothState.entries.filter(entry=>entry.id!==id);
  document.dispatchEvent(new CustomEvent("dental-chart:delete-entry",{detail:{id}}));
...
function surfaceClipPath(n,v){
  ...
  const photoKey=`${isPrimaryTooth(n)?"primary":chartMode}:${n}:${v}`;
...
function usesWholeTreatmentBand(treatment){
  return treatment==="bridge"||treatment==="extraction"||treatment==="implant"||treatment==="crown";
}
...
function wholeStatusOverlaySVG(n, v, treatment, status, entry = draft, colorTreatment = treatment) {
  if(treatment==="extraction")return extractionCrossSVG(n,v);
  if(treatment==="retainedRoot")return "";
  if(treatment==="implant")return wholeStatusOverlaySVG(n,v,"crown",status,{...entry,treatment:"implant"},"implant");
  const dims =
    v === "front"
  ...
  if (["bridge", "partialDenture", "crown", "extraction", "implant", "veneer"].includes(treatment)) {
    ...
    const color = entryColor({...entry,treatment:colorTreatment});
    const stroke = darken(color, 14);
    const planned = status === "planned";
    const review = status === "watch";
    const opacity = review ? "0.20" : status === "preview" ? "0.55" : "0.82";
    const prostheticPath = surfaceClipPath(n, v);
    const crownOnly = treatment === "bridge" || treatment === "partialDenture" || treatment === "crown" || treatment === "veneer";
    const bounds=crownBounds(n,v);
    const crownHeight = crownOnly ? bounds.end : dims.height;
    const cervicalY = crownHeight - 2;
    const crownRegion = crownOnly ? `M0,${bounds.start} H${dims.width} V${bounds.end} H0 Z` : `M0,0 H${dims.width} V${dims.height} H0 Z`;
    ...
    if(treatment==="partialDenture"){
      const grouped=entry.bridgeId?flatEntries().filter(item=>item.bridgeId===entry.bridgeId&&item.treatment==="partialDenture").map(item=>item.tooth):selection.teeth;
      const targets=groupedTargets(grouped.length?grouped:[n],"partialDenture");
      const first=n===targets[0],last=n===targets[targets.length-1];
      // Work from the biting edge toward the gum; lower occlusal art is reversed.
      const w=dims.width,h=bounds.end-bounds.start;
      const reverse=v==="occ"&&bounds.start>0;
      const origin=reverse?bounds.end:bounds.start;
      const gumId=`rpd-gum-${serial}`,metalId=`rpd-metal-${serial}`;
      const neck=h*.82,bottom=h+5,left=first?-1:-3,right=last?w+1:w+3;
  ...
  if (usesWholeTreatmentBand(treatment)) {
    ...
    const color = entryColor({...entry,treatment});
    const stroke = darken(color, 14);
...
function renderSurfaceOverlay(n,v,{complete={},planned={},review={},selected=new Set(),previewColor=null,preview=false,clipKey='chart'}={}){
  const dims=v==='front'?{width:toothW(n),height:toothH(n)}:crownDims(n),clipId='surface-'+clipKey+'-'+n+'-'+v;
  const defs=surfaceDefs(n,v);
  return '<svg class="surface-svg" width="'+dims.width+'" height="'+dims.height+'" viewBox="0 0 '+dims.width+' '+dims.height+'"><defs><clipPath id="'+clipId+'">'+surfaceClipPath(n,v)+'</clipPath></defs><g clip-path="url(#'+clipId+')">'+defs.map((def,index)=>{
    const entry=planned[def.key]||complete[def.key]||review[def.key];
    const treatment=typeof entry==='string'?entry:entry?.treatment;
    const isSel=selected.has(def.key);
    const color=entry?entryColor(entry):(preview&&isSel?previewColor:null);
    const damage=treatment==='fracture'||treatment==='crack';
    const sealant=treatment==='sealant';
    const regionId=clipId+'-'+index;
    const scale=treatment==='inlay'?.54:treatment==='onlay'?.78:1;
    const transform=scale===1?'':' transform="translate('+def.cx+' '+def.cy+') scale('+scale+') translate('+(-def.cx)+' '+(-def.cy)+')"';
    const shape='<path d="'+def.path+'" fill="'+(damage||sealant?'transparent':color||'transparent')+'" fill-opacity="'+(entry?.92:preview&&isSel?.5:0)+'"'+transform+'/>';
    const crackPath='M'+(def.cx-5)+' '+(def.cy-11)+' l5 6 -3 5 7 5 -4 7 M'+(def.cx+1)+' '+(def.cy+1)+' l7 -5';
    const defect=treatment==='fracture'?'<path d="M'+(def.cx-7)+' '+(def.cy-12)+' l9 7 -4 5 8 5 -6 9 -7 -10 3 -5 -6 -5 Z" fill="#0d1725"/>':'';
    const mark=damage?'<defs><clipPath id="'+regionId+'"><path d="'+def.path+'"/></clipPath></defs><g clip-path="url(#'+regionId+')">'+defect+'<path class="damage-shadow" d="'+crackPath+'" fill="none" stroke="#111827" stroke-width="'+(treatment==='fracture'?6:4)+'"/><path class="damage-'+treatment+'" d="'+crackPath+'" fill="none" stroke="'+(treatment==='fracture'?'#fb923c':'#fde047')+'" stroke-width="'+(treatment==='fracture'?3:1.8)+'"/></g>':'';
    const sealantMark=sealant?'<defs><clipPath id="'+regionId+'"><path d="'+def.path+'"/></clipPath></defs><g clip-path="url(#'+regionId+')"><path class="sealant-groove" style="--sealant-color:'+color+'" d="M'+(def.cx-6)+' '+def.cy+' Q'+def.cx+' '+(def.cy-3)+' '+(def.cx+6)+' '+def.cy+' M'+def.cx+' '+(def.cy-6)+' Q'+(def.cx+2)+' '+def.cy+' '+def.cx+' '+(def.cy+6)+'" fill="none" stroke="'+color+'" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></g>':'';
    return shape+mark+sealantMark+'<path class="surface-region'+(isSel?' selected':'')+'" data-surface="'+def.key+'" d="'+def.path+'" fill="transparent" stroke="'+(isSel?'#8fc9ff':'transparent')+'" stroke-width="'+(isSel?2.5:0)+'"/>';
  }).join('')+'</g></svg>';
}
function surfaceOverlaySVG(n,v,preview=false){
  ...
  const selected=preview?new Set(draft.surfaces):(selectedSurfaces(n,v));
...
function renderNumbersRow(container, list) {
  ...
  const statusContext =
    container === els.splitPlannedUpperNumbers ||
    container === els.splitPlannedLowerNumbers
      ? "planned"
      : "existing";
  list.forEach(n => {
    const ghost = n !== null && isGhostPrimarySlot(n);
    const clickable = n !== null && !ghost;
    const item = document.createElement(clickable ? "button" : "div");

    item.className =
      `number${n === null ? " empty" : ""}${ghost ? " ghost" : ""}`;
    item.textContent = n === null ? "" : n;

    if (clickable) {
      item.type = "button";
      item.setAttribute("aria-label", `Select tooth ${n}`);
      const canSwap = Boolean(swapPrimaryToothNumber(n));
      let clickTimer;
      ...
      if (canSwap) {
        item.title = "Double-click to switch between primary and permanent";
  ...
  const layer=container===els.upperNumbers||container===els.lowerNumbers?"combined":statusContext;
  appendSpacingMarkers(container,list,"numbers",layer);
}
function spacingEntriesForLayer(n,layer){
  const status=layer==="planned"?"planned":"existing";
  return entriesForTooth(n).filter(entry=>entry.treatment==="spacing"&&entryLayer(entry)===status);
}
function appendSpacingMarkers(container,list,view,layer){
  if(view!=="front"&&view!=="occ"&&view!=="numbers")return;
  const teeth=[...container.children];
  const gaps=new Set();
...
// Normalize the row structure even when an older HTML shell was cached.
function ensureFourArchRows(){
  if(els.splitStage.dataset.layout==="four-arches")return;
  const rows=[["planned","upper"],["existing","upper"],["existing","lower"],["planned","lower"]];
  const sections=rows.map(([layer,arch])=>{
    const grid=document.getElementById(`split-${layer}-${arch}-front`).closest(".arch-grid");
    const section=document.createElement("section");
    const status=layer==="planned"?"Planning":"Existing";
    section.className="split-group";
    section.dataset.chartLayer=layer;
    section.setAttribute("aria-label",`${status} ${arch} arch`);
    grid.querySelector(".arch-label-text").innerHTML=`<span class="split-status-label ${layer}">${status}</span><span>${arch==="upper"?"Upper":"Lower"} Arch</span>`;
    section.appendChild(grid);
    return section;
  });
  els.splitStage.replaceChildren(...sections);
  els.splitStage.dataset.layout="four-arches";
}
...
function renderBridgeMiniPreview(){
  ...
  els.miniPreview.innerHTML=`<div class="mini-art mini-bridge-preview"><div class="mini-bridge-row" style="--bridge-count:${targets.length};--bridge-scale:${scale}">${targets.map((n,index)=>{
    ...
    const modeClass=`front ${isUpper(n)?'upper':'lower'}`;
    return `<div class="mini-bridge-tooth" data-tooth="${n}" data-role="${role}"><div class="art-core ${modeClass}">${anatomySVG(n,'front',entry)}${wholeStatusOverlaySVG(n,'front','bridge',draft.status==='planned'?'planned':'existing',entry)}</div></div>`;
  }).join('')}</div></div>`;
  els.miniPreview.querySelectorAll('.mini-bridge-tooth').forEach((node,index)=>{
    const n=targets[index],entry={...draft,treatment:'bridge',bridgeRole:node.dataset.role};
...
function renderMiniPreview(){els.miniPreview.classList.remove("empty"); const t=treatmentFor(draft.treatment),whole=t.mode==="whole",missing=draft.treatment==="missing"; if(draft.treatment==="bridge"){renderBridgeMiniPreview();return} if(missing){els.miniPreview.innerHTML=missingSVG(draft.tooth,draft.view); return} const previewView=draft.treatment==="perioPocket"?"front":draft.view; const baseSVG=anatomySVG(draft.tooth,previewView,draft); let html=baseSVG; if(draft.treatment==="perioPocket")html+=perioPocketOverlaySVG(draft.tooth,previewView); if(whole) html+=wholeStatusOverlaySVG(draft.tooth,draft.view,draft.treatment,draft.status==="planned"?"planned":draft.status==="watch"?"watch":"existing"); if(t.mode==="root") html+=rctOverlayHTML(draft.tooth); if(t.mode==="surface"){const map={}; draft.surfaces.forEach(s=>map[s]={...draft}); html+=renderSurfaceOverlay(draft.tooth,draft.view,{complete:draft.status==="existing"?map:{},planned:draft.status==="planned"?map:{},review:draft.status==="watch"?map:{},selected:new Set(),previewColor:null,preview:false,clipKey:"draft-mini"})} const modeClass=previewView==="front"?`front ${isUpper(draft.tooth)?"upper":"lower"}`:"occ"; const reviewBadge=draft.status==="watch"?reviewBadgeHTML(draft.tooth,previewView):""; const conditionBadge=t.mode==="label"&&draft.treatment!=="perioPocket"?conditionBadgeHTML(draft,'mini-condition-badge'):""; els.miniPreview.innerHTML=`<div class="mini-art ${modeClass}"><div class="art-core ${modeClass}" style="transform:${miniPreviewTransform(draft.tooth,previewView)}">${html}</div>${reviewBadge}${conditionBadge}</div>`; applyAnatomyClip(els.miniPreview.querySelector(".art-core"),draft.tooth,previewView,draft)}
...
function bridgeSelectionError(teeth){
  const unique=[...new Set(teeth)];
  if(unique.length<2)return 'Select two bridge endpoints in the same arch.';
  const arch=[activeUpperDisplay(),activeLowerDisplay()].find(row=>unique.every(n=>row.includes(n)));
  if(!arch)return 'Select bridge endpoints in the same arch.';
  const positions=unique.map(n=>arch.indexOf(n));
  const span=arch.slice(Math.min(...positions),Math.max(...positions)+1);
  if(span.length<3)return 'Select endpoints with at least one tooth between them.';
  if(span.some(n=>!n||isGhostPrimarySlot(n)))return 'The bridge cannot cross an inactive tooth or empty chart slot.';
  return '';
}
function bridgeSpan(teeth){
  if(bridgeSelectionError(teeth))return [...teeth];
  const arch=[activeUpperDisplay(),activeLowerDisplay()].find(row=>teeth.every(n=>row.includes(n)));
  const positions=teeth.map(n=>arch.indexOf(n));
...
function partialDentureSelectionError(teeth){
  const unique=[...new Set(teeth)];
  if(!unique.length)return 'Select at least one replacement tooth for the partial denture.';
  const arch=[activeUpperDisplay(),activeLowerDisplay()].find(row=>unique.every(n=>row.includes(n)));
  if(!arch)return 'Select partial-denture teeth in the same arch.';
  const positions=unique.map(n=>arch.indexOf(n)).sort((a,b)=>a-b);
  if(positions.at(-1)-positions[0]+1!==positions.length)return 'Select consecutive replacement teeth for one partial-denture saddle.';
  if(unique.some(n=>!n||isGhostPrimarySlot(n)))return 'The partial denture cannot use an inactive tooth or empty chart slot.';
  return '';
}
...
function renderBridgeSelectionHint(){
  let hint=document.getElementById("bridge-selection-hint");
  const selectionField=els.selectedTeeth.closest('.selection-field');
  const title=selectionField?.querySelector('.field-title');
  if(title){
    title.classList.add('selection-title-row');
    if(!title.querySelector('.selection-title-text')){
      const label=document.createElement('span');label.className='selection-title-text';label.textContent=title.textContent.trim()||'Selected teeth';title.replaceChildren(label);
    }
  }
  if(hint&&!hint.classList.contains('bridge-hint-icon')){hint.remove();hint=null}
  if(!hint){
    hint=document.createElement("button");hint.type="button";hint.id="bridge-selection-hint";hint.className="bridge-hint-icon";
    hint.innerHTML='<svg viewBox="0 0 20 20" aria-hidden="true"><circle cx="10" cy="10" r="8"/><path d="M10 9v5 M10 6.2v.2"/></svg>';
    title?.append(hint);
  }
...
function scheduleBridgeConnectors(){
  cancelAnimationFrame(scheduleBridgeConnectors.frame);
...
function renderBridgeConnectors(){
  ...
  return;
  /* Original connector rendering retained for reference.
  document.querySelectorAll(".teeth-row[data-bridge-teeth]").forEach(row=>{
    row.querySelector(".bridge-connectors")?.remove();
    const list=JSON.parse(row.dataset.bridgeTeeth),layer=row.dataset.bridgeLayer;
    ...
    list.forEach((n,index)=>{
      const next=list[index+1];if(!n||!next)return;
      const a=latestWhole(n,layer),b=latestWhole(next,layer);
...
(function desktopEditorSelection(){
  ...
  const fields=editor?.querySelector('.editor-fields');
  const selectedHead=editor?.querySelector('.selected-head');
  const selectedMain=editor?.querySelector('.selected-main');
  const preview=selectedMain?.querySelector('.preview');
  const selectionField=fields?.querySelector('.selection-field');
  const batch=document.getElementById('multi-toggle-btn');
  if(!editor||!fields||!selectedHead||!selectedMain||!preview||!selectionField||!batch)return;
  const selectionAnchor=document.createComment('selection original position');
  selectionField.before(selectionAnchor);
