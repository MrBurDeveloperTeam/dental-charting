...
import { listAllPatientRecords, type PatientRecord } from "../services/patientRecords";
...
export function PatientRecordsPage({ onOpenRecord }: { onOpenRecord: (record: PatientRecord) => void }) {
  ...
  useEffect(() => { let active = true; Promise.all([listAllPatientRecords(), listDentalDentists()]).then(([rows, staff]) => { if (active) { setRecords(rows); setDentists(staff); } }).catch((reason: unknown) => active && setError(reason instanceof Error ? reason.message : "Unable to load patient records.")).finally(() => active && setLoading(false)); return () => { active = false; }; }, []);
  ...
  return <main className="patient-records-page">
    <header className="patient-records-heading"><h2>Patient Records</h2><p>All dental chart records for this account</p></header>
    ...
    <section className="patient-record-table-card"><div className="patient-record-table-scroll"><table className="patient-record-table"><thead><tr>
      ...
      {loading && <tr><td className="patient-record-empty" colSpan={9}>Loading patient records…</td></tr>}{error && <tr><td className="patient-record-empty error" colSpan={9}>{error}</td></tr>}
