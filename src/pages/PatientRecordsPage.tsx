...
export function PatientRecordsPage({ onOpenRecord }: { onOpenRecord: (record: PatientRecord) => void }) {
  ...
  return <main className="patient-records-page">
    ...
    <section className="patient-records-toolbar"><label className="patient-record-search"><span>⌕</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by name, IC/ID, phone or date..." /></label>
