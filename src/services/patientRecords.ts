...
export async function listAllPatientRecords(): Promise<PatientRecord[]> {
  const supabase = getSupabaseClient();
  const { clinicId } = await getClinicSession();
  const { data: visits, error: visitError } = await supabase.from("dental_chart_visits")
    .select("id,patient_id,appointment_id,dentist_id,visit_date,created_at").eq("clinic_id", clinicId);
  if (visitError) throw visitError;
  if (!visits?.length) return [];

  const visitIds = visits.map((visit) => visit.id);
  const { data: chartEntries, error: entryError } = await supabase.from("dental_chart_entries")
    .select("id,chart_visit_id,dentition,tooth_number,treatment,status,updated_at").in("chart_visit_id", visitIds).order("updated_at", { ascending: false });
  ...
  for (const entry of chartEntries as PatientRecordEntry[]) { const rows = entriesByVisit.get(entry.chart_visit_id) || []; rows.push(entry); entriesByVisit.set(entry.chart_visit_id, rows); }
