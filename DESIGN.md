...
The treatment editor follows surface → condition/treatment → material → status → clinical note. Within Condition / treatment, groups appear as Condition → Restoration → Procedure → Prosthetic. Chart-type and view controls remain commented in the markup; the existing category and view models still drive chart geometry and persistence. Material swatches use a three-column grid, while condition/treatment buttons use neutral text and category headings. The name/color-only editor reuses the existing modal and saves the clinic catalogue through `src/services/dentalMaterials.ts`.

The Material field is progressive: selecting any item in the Condition group hides the field and clears its draft value because conditions do not carry restorative material. Restoration, Procedure, and Prosthetic selections keep Material available.
...
Impacted teeth use an upright `IMP` mark centered directly on the tooth rather than the external condition-badge stack. Dark board-ink lettering with a crisp light outline keeps the abbreviation readable over enamel, materials, and surface colors without introducing another colored fill; other label conditions retain the external badge treatment.

Built-in materials are a shared frontend catalogue referenced from chart entries by stable `material_code`. The `dental_material` table is sparse: it stores only clinic-specific built-in overrides and clinic-created materials. Custom materials retain UUID references in `dental_chart_entries.material`; restoring a built-in to its default removes the clinic override.
